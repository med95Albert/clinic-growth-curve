...
| 診所伺服器（常開的那台 Windows） | 裝好 Claude Code（它會要求 Git for Windows）；有 Python 3.12 更好，沒有的話 agent 會請你允許 UAC 用 winget 裝。repo **部署期間為公開**，clone 免登入；部署完成後改回私有（見文末），之後更新才需要 `gh auth login` 一次 |
...
```bash
gh repo edit med95Albert/clinic-growth-curve --visibility private --accept-visibility-change-consequences
```

改回私有後，診所伺服器的 `git pull` 會要求登入：跑一次 `gh auth login` 即可（runbook Step 0 有寫）。repo 已排除 `bench/`（實拍照片）、`pilot/`（試用留底）與所有 `.env`，不含任何病人資料；文件內只有私有網段 IP。
